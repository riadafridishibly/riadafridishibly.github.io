...
<!-- Refactor This -->
Let's learn some terminology. We'll call our data (what we want to encrypt) `plaintext`. We'll encrypt the `plaintext` and get `ciphertext`. We'll use a `key` which is our secret. So, we'll encrypt `plaintext` using the `key` and get `ciphertext`.
...
Now we've all things set. Let's encrypt.
